...
class ShapeManager(object):
    ...
    def get_enum(self, data, name1, name2 = ''):
        ...
        return np.array(indices).reshape(-1,len(set(names)))
...
class InputShapeManager(ShapeManager):
    def __init__(self):
        ...
        self.trajectory_action = self.get_enum(self.file, 'Trajectory')
        ...
        self.goal_action = self.get_enum(self.file, 'Action')
...
class DataBase(object):
    ...
    def normed(self):
        return (self.data - self.mean) / (1e-5 + self.std)
...
if __name__ == "__main__":
    ...
    x = Data(data, norm, OutputShapeManager())
    print (x.goal)
